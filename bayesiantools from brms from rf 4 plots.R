...
library(brms)          # For Bayesian regression using brms
...
mcmc_model <- brm(
  ...
  data = df_filtered,
  family = gaussian(),
  prior = set_prior("normal(0, 1)", class = "b"),  # Example of an informative prior
  iter = 4000,  # Number of iterations
  chains = 4,   # Number of chains
  seed = 123,
  control = list(adapt_delta = 0.99)  # Increased adapt_delta to reduce divergent transitions
)
...
aic <- n * log(rss/n) + 2 * length(fixef(mcmc_model))  # AIC approximation
bic <- n * log(rss/n) + log(n) * length(fixef(mcmc_model))  # BIC approximation
