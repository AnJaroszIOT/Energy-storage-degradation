...
stan_data <- list(
  ...
  Decrement_3_6_3_4V = df_filtered$Decrement_3.6_3.4V,  # Updated variable name
...
# MCMCpack model
mcmc_model <- MCMCregress(Max_Voltage_Dischar ~ Discharge_Time + Decrement_3.6_3.4V + Time_constant_current, data = df_filtered)
summary(mcmc_model)
...
model {
  for (i in 1:N) {
    ...
    mu[i] <- alpha + beta1 * Discharge_Time[i] + beta2 * Decrement_3_6_3_4V[i] + beta3 * Time_constant_current[i];
  }
  alpha ~ dnorm(0, 0.001);
  beta1 ~ dnorm(0, 0.001);
  beta2 ~ dnorm(0, 0.001);
...
# brms model
brms_model <- brm(Max_Voltage_Dischar ~ Discharge_Time + Decrement_3.6_3.4V + Time_constant_current, data = df_filtered, family = gaussian(), iter = 2000, chains = 4)
summary(brms_model)
