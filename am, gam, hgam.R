...
# Assuming a more complex structure, you might want to include random slopes or additional random effects
# Here is an example of a more complex hierarchical model
...
model_summaries <- data.frame(
  Model = c("Linear", "GLM", "Hierarchical", "Additive", "GAM", "Hierarchical Advanced"),
  AIC = c(AIC(linear_model), AIC(glm_model), AIC(glmer_model), AIC(additive_model), AIC(gam_model), AIC(hglm_model)),
  BIC = c(BIC(linear_model), BIC(glm_model), BIC(glmer_model), BIC(additive_model), BIC(gam_model), BIC(hglm_model)),
  WAIC = c(calculate_waic(linear_model), calculate_waic(glm_model), calculate_waic(glmer_model), calculate_waic(additive_model), calculate_waic(gam_model), calculate_waic(hglm_model))
)
...
# Hierarchical Advanced Model Plot
df_filtered$Predicted_HGLM <- predict(hglm_model)
plot_hgam <- ggplot(df_filtered, aes(x = Discharge_Time, y = Predicted_HGLM)) +
  geom_point(aes(y = Max_Voltage_Dischar), color = "blue") +
  geom_line(color = "darkred") +
  labs(title = "Hierarchical Generalized Advanced Model", x = "Discharge Time", y = "Max Volt Dischar") +
  theme_minimal() +
  theme(axis.title.y = element_text(size = 10))  # Adjust y-axis title size

# Arrange plots in a grid with six rows
grid.arrange(plot_additive, plot_gam, plot_hgam, nrow = 3)