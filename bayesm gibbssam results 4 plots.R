...
library(bayesm)  # For Gibbs Sampling
...
for (i in 1:n_iter) {
  ...
  beta_var <- solve(t(X) %*% X / sigma2_init + diag(1, p))
  beta_mean <- beta_var %*% (t(X) %*% y / sigma2_init)
  beta_samples[i, ] <- mvrnorm(1, beta_mean, beta_var)
  
  # Sample from the posterior of sigma2
  residuals <- y - X %*% beta_samples[i, ]
  shape <- (n / 2) + 1
  scale <- sum(residuals^2) / 2
  sigma2_samples[i] <- 1 / rgamma(1, shape, scale)
}
