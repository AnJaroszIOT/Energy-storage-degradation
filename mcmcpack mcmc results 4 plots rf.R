...
mcmc_model <- MCMCregress(Max_Voltage_Dischar ~ Discharge_Time + Decrement_3.6_3.4V + Time_constant_current,
                          ...
                          mcmc = 10000,   # Total number of MCMC iterations
                          thin = 10)      # Thinning factor

# Calculate the mean of the posterior samples for coefficients
coefficients <- colMeans(mcmc_model)

# Generate predictions for each observation in df_filtered
...
# Check if the length of predictions matches the number of rows in df_filtered
if (length(predictions) != nrow(df_filtered)) {
  stop("The length of predictions does not match the number of rows in df_filtered.")
}
...
aic <- n * log(rss/n) + 2 * length(coefficients)  # AIC approximation
bic <- n * log(rss/n) + log(n) * length(coefficients)  # BIC approximation

# Summaries for AIC, BIC, and WAIC (WAIC is not applicable here)
aic_bic_waic_summary <- data.frame(
  Metric = c("AIC", "BIC", "WAIC"),
  Value = c(aic, bic, NA)  # WAIC is not applicable for MCMC
)
...
cat("\nSummary for Plot 1 (Predictions):\n")
print(summary_p1)

cat("\nSummary for Plot 2 (Decrement vs Max Voltage Discharge):\n")
print(summary_p2)

cat("\nSummary for Plot 3 (Discharge Time vs Max Voltage Discharge):\n")
print(summary_p3)

cat("\nSummary for Plot 4 (Time Constant Current vs Max Voltage Discharge):\n")
print(summary_p4)
